...
inline constexpr uint32_t invalid_slot {0xFFFFFFFFU};
...
public:
	...
	T& add(Entity entity, Types&&... args)
	{
		...
		if (entity >= rack.sparse_index.size())
			rack.sparse_index.resize(static_cast<size_t>(entity) + 1U, cfg::invalid_slot);

		uint32_t slot = rack.sparse_index[entity];
		if (slot == cfg::invalid_slot) {
	...
	void remove(Entity entity)
	{
		...
		rack.sparse_index[entity] = cfg::invalid_slot;
	}
...
private:
	...
	struct Rack
	{
		...
		void erase_entity(Entity entity)
		{
			...
			sparse_index[entity] = cfg::invalid_slot;
		}
		...
		void clear()
		{
			...
			sparse_index.resize(0, cfg::invalid_slot);
		}
	};
