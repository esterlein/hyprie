...
#define HPR_PANIC_FMT(format, ...) \
	::hpr::fail::panic_fmt(__FILE__, __LINE__, format, ##__VA_ARGS__)
...
#define HPR_ASSERT_MSG(expr, format, ...) \
	do { \
		if (!(expr)) [[unlikely]] { \
			::hpr::fail::panic_fmt(__FILE__, __LINE__, format, ##__VA_ARGS__); \
		} \
...
#define HPR_ASSERT_MSG(expr, format, ...) ((void)0)
