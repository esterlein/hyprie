...
inline constexpr float parallel_epsilon      = 1e-6f;
inline constexpr float zero_div_epsilon      = 1e-8f;


inline constexpr size_t frustum_plane_cnt = 6;


inline constexpr float aabb_plane_half_thickness = 0.01f;


static inline std::array<vec4, frustum_plane_cnt> frustum_planes(const mat4& mtx_VP)
{
