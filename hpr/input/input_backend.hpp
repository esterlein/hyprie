...
inline void handle_event(const sapp_event* event, InputState& state)
{
	...
	case SAPP_EVENTTYPE_KEY_DOWN: {
		...
		if (event->key_code == SAPP_KEYCODE_J)  { state.key_j = true; }
		if (event->key_code == SAPP_KEYCODE_K)  { state.key_k = true; }
	...
	case SAPP_EVENTTYPE_KEY_UP: {
		...
		if (event->key_code == SAPP_KEYCODE_J) { state.key_j = false; }
		if (event->key_code == SAPP_KEYCODE_K) { state.key_k = false; }
