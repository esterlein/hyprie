...
public:
	...
	void map(const hpr::io::InputState& input_state, mtp::vault<Action, mtp::default_set>& current_actions)
	{
		...
		if (input_state.key_k) {
			DebugScrollUp payload {};
			current_actions.push_back({ActionKind::DebugScrollUp, payload});
		}
		if (input_state.key_j) {
			DebugScrollDown payload {};
			current_actions.push_back({ActionKind::DebugScrollDown, payload});
		}
