...
{
	const vec3 w = ray_origin - axis_origin;
	const float b = glm::dot(ray_dir_unit, axis_dir_unit);
	const float c = glm::dot(axis_dir_unit, axis_dir_unit);
	const float d = glm::dot(ray_dir_unit, w);
	const float e = glm::dot(axis_dir_unit, w);

	const float denom = 1.0f * c - b * b;
	float axis_coord = 0.0f;

	if (std::abs(denom) > math::collinearity_epsilon) {
		axis_coord = (1.0f * e - b * d) / denom;
	} else {
		axis_coord = e;
	}

	return RayAxisProjection{ axis_coord, 0.0f };
}
...
)
{
	...
	const float axis_len_world    = world_units_per_pixel * style.axis_len_px;
	...
	AxisCandidate axis_candidate { GizmoAxis::Screen, std::numeric_limits<float>::max(), 0.0f };

	auto test_axis = [&](const vec3& axis_dir, GizmoAxis label) {
		const RayAxisProjection proj = project_ray_onto_axis(ray_origin, ray_dir, gizmo_origin_world, axis_dir);
		
		const float t_axis = std::clamp(proj.closest_axis_coord, 0.0f, axis_len_world);
		const vec3 p_axis = gizmo_origin_world + axis_dir * t_axis;
		
		const float t_ray = std::max(0.0f, glm::dot(p_axis - ray_origin, ray_dir));
		const vec3 p_ray = ray_origin + ray_dir * t_ray;

		const float dist_world = glm::length(p_axis - p_ray);
		if (dist_world <= axis_radius_world) {
			const float px = dist_world / world_units_per_pixel;
			if (px < axis_candidate.distance_px)
				axis_candidate = { label, px, t_axis };
		}
	};
	...
	PlaneCandidate best_plane { GizmoAxis::Screen, std::numeric_limits<float>::max(), 0.0f };

	auto test_plane = [&](const vec3& axis_u, const vec3& axis_v, GizmoAxis label) {
		const vec3 plane_normal = glm::cross(axis_u, axis_v);
		float hit_dist = 0.0f;
		vec3 hit_pt {};

		if (!intersect_ray_with_plane(ray_origin, ray_dir, gizmo_origin_world, plane_normal, hit_dist, hit_pt))
			return;

		if (!point_in_plane_square(hit_pt, gizmo_origin_world, axis_u, axis_v, plane_half_world))
			return;

		const float dist_world = glm::length(hit_pt - gizmo_origin_world);
		const float px = dist_world / world_units_per_pixel;

		if (px < best_plane.distance_px)
			best_plane = { label, px, hit_dist };
	};
