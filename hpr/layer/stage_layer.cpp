...
bool StageLayer::on_actions(const scn::SceneContext& scene_ctx, std::span<const io::Action> actions)
{
	(void) scene_ctx;

	bool action_consumed = false;
	for (const io::Action& action : actions) {
		if (action.kind != io::ActionKind::SelectClick) continue;
...
void StageLayer::on_submit(const scn::SceneContext& scene_ctx, uint32_t layer_idx)
{
	...
	auto& cue_trs_mass = m_staging_ctx.cue_blob_mass;
	auto& orl_trs_mass = m_staging_ctx.orl_blob_mass;
	...
	m_registry.template scan<ecs::LightComponent, ecs::TransformComponent>(
		...
		)
		{
			...
			const uint32_t vtx_base  = m_canonical.vtx_base_solid  + box_submesh.vtx_base;
			const uint32_t idx_first = m_canonical.idx_first_solid + box_submesh.idx_first;
			...
			m_cue_queue.push(rdr::CueDrawCmd {
				...
				.layer_idx     = layer_idx,
				.vtx_base      = m_canonical.vtx_base_solid,
				.idx_first     = idx_first,
				.idx_count     = box_submesh.idx_count,
				.blob_idx       = cue_trs_idx,
				.cue_mask      = cue_mask,
			...
			const uint64_t orl_sort_key =
					...
					(static_cast<uint64_t>(m_canonical.vtx_base_solid & 0xFFFFU)   << 32) |
					(static_cast<uint64_t>(idx_first                  & 0xFFFFFFU) << 8)  |
