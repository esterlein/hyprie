...
bool GizmoLayer::on_actions(const scn::SceneContext& scene_ctx, std::span<const Action> actions)
{
	...
	for (const Action& action : actions) {
		...
		case ActionKind::GizmoUpdate:
		{
			...
			auto world_to_px = [this, &draw_view](const vec3& position_world) -> vec2
			{
				vec4 clip = draw_view.mtx_VP * vec4(position_world, 1.0f);
				vec3 ndc_coords = vec3(clip) / clip.w;

				return vec2 {
					(ndc_coords.x * 0.5f + 0.5f)          * static_cast<float>(m_surface_info.width),
					(1.0f - (ndc_coords.y * 0.5f + 0.5f)) * static_cast<float>(m_surface_info.height)
				};
			};
			...
			if (m_mode == edt::GizmoMode::Translate) {
				...
				{
					const mat3 inv_view3 = mat3(glm::inverse(draw_view.mtx_V));
			...
			if (m_mode == edt::GizmoMode::Rotate) {
				...
				](const vec3& axis_dir_unit) -> bool
				{
					...
					if (start_hit_valid && curr_hit_valid) {
						...
						const mat3 mtx_view_inv = mat3(glm::inverse(draw_view.mtx_V));
...
void GizmoLayer::on_update(scn::SceneContext& scn_ctx, float delta_time)
{
	(void) scn_ctx;
	(void) delta_time;
...
void GizmoLayer::on_submit(const scn::SceneContext& scene_ctx, uint32_t layer_idx)
{
	if (m_entity == ecs::ctx::invalid_entity)
		return;

	m_screen_scale = rdr::world_size_per_px(
		m_position,
		scene_ctx.draw_view.mtx_V,
		scene_ctx.draw_view.mtx_P,
		m_surface_info.height
	);
	...
	const mat4 base_rot = (m_space == edt::TransformSpace::Local)
	...
	[
		...
		layer_idx,
		&overlay_trs_mass
	](uint32_t submesh_idx, const mat4& model, const vec4& color)
	{
		const auto geo_range     = m_gizmo_primitives.geo_range[submesh_idx];
		const uint32_t trs_idx   = overlay_trs_mass->push_raw({model, color});
		const uint32_t vtx_base  = m_gizmo_primitives.vtx_base  + geo_range.vtx_base;
		const uint32_t idx_first = m_gizmo_primitives.idx_first + geo_range.idx_first;
		...
		const uint64_t orl_sort_key =
			...
			(static_cast<uint64_t>(geo_range.idx_count & 0xFFU));

		m_cmd_queue.push(rdr::OverlayDrawCmd {
	...
	case edt::GizmoMode::Translate: {
		...
		push(static_cast<uint32_t>(edt::GizmoSubmesh::Arrow), base * rot_x * scale_axis, col_x);
		push(static_cast<uint32_t>(edt::GizmoSubmesh::Arrow), base * rot_y * scale_axis, col_y);
		push(static_cast<uint32_t>(edt::GizmoSubmesh::Arrow), base * rot_z * scale_axis, col_z);

		push(static_cast<uint32_t>(edt::GizmoSubmesh::Quad), base * rot_xy * scale_plane, col_plane);
		push(static_cast<uint32_t>(edt::GizmoSubmesh::Quad), base * rot_xz * scale_plane, col_plane);
		push(static_cast<uint32_t>(edt::GizmoSubmesh::Quad), base * rot_yz * scale_plane, col_plane);
	}
	break;

	case edt::GizmoMode::Rotate: {

		push(static_cast<uint32_t>(edt::GizmoSubmesh::Ring), base * rot_xy * scale_ring, col_x);
		push(static_cast<uint32_t>(edt::GizmoSubmesh::Ring), base * rot_xz * scale_ring, col_y);
		push(static_cast<uint32_t>(edt::GizmoSubmesh::Ring), base * rot_yz * scale_ring, col_z);
	}
	break;

	case edt::GizmoMode::Scale: {

		push(static_cast<uint32_t>(edt::GizmoSubmesh::Cube), base * rot_x * translate_tip * scale_tip, col_x);
		push(static_cast<uint32_t>(edt::GizmoSubmesh::Cube), base * rot_y * translate_tip * scale_tip, col_y);
		push(static_cast<uint32_t>(edt::GizmoSubmesh::Cube), base * rot_z * translate_tip * scale_tip, col_z);
	}
	break;
