...
layout(set = 0, binding = 5) uniform texture2D u_tex_palette;
layout(set = 0, binding = 6) uniform sampler   u_smp_palette;

layout(set = 0, binding = 7) uniform u_cue_params
{
	int mask;
	int palette;
	int tilemap;
...
void main()
{
	...
	float palette_sample_x = (float(u_cue.tilemap) + 0.5) / 256.0;
