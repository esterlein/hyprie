...
layout(location = 4) in uvec4 jnt_in;
layout(location = 5) in vec4  wgt_in;
...
layout(set = 0, binding = 14, std430) readonly buffer ssbo_bones
{
	Bones bones[];
};
...
void main()
{
	uint pose_idx = trs[base_inst_idx + gl_InstanceIndex].base_pose_idx;

	mat4 skin_mat =
		wgt_in.x * bones[pose_idx + jnt_in.x].mtx +
		wgt_in.y * bones[pose_idx + jnt_in.y].mtx +
		wgt_in.z * bones[pose_idx + jnt_in.z].mtx +
		wgt_in.w * bones[pose_idx + jnt_in.w].mtx;

	mat4 mtx_M = trs[base_inst_idx + gl_InstanceIndex].mtx_M * skin_mat;
	mat4 mtx_N = trs[base_inst_idx + gl_InstanceIndex].mtx_N * skin_mat;
